export const AGGREGATOR_ID = 'cEZfKpbSfHYrmaOFGzn7CvHgWdHueZEn9DHq-aLpCms';

export const AO_TOKEN = {
  name: 'AO',
  ticker: 'AO',
  denomination: 12,
  logo: 'UkS-mdoiG8hcAClhKK8ch4ZhEzla0mCPDOix9hpdSFE',
  processId: '0syT13r0s0tgPmIed95bJnuSqaD29HQNN8D3ElLSrsc',
};

export const USDC_TOKEN = {
  processId: '7zH9dlMNoxprab9loshv3Y7WG45DOny_Vrq9KrXObdQ',
  ticker: 'wUSDC',
  name: 'Ethereum-Wrapped USDC',
  denomination: 6,
  logo: 'VL4_2p40RKvFOQynAMilDQ4lcwjtlK3Ll-xtRhv9GSY',
};

export const TRUNK_TOKEN = {
  name: 'TRUNK',
  ticker: 'TRUNK',
  denomination: 3,
  logo: 'hqg-Em9DdYHYmMysyVi8LuTGF8IF_F7ZacgjYiSpj0k',
  processId: 'wOrb8b_V8QixWyXZub48Ki5B6OIDyf_p1ngoonsaRpQ',
};

export const LLAMA_TOKEN = {
  name: 'Llama Coin',
  ticker: 'LLAMA',
  denomination: 12,
  logo: '9FSEgmUsrug7kTdZJABDekwTGJy7YG7KaN5khcbwcX4',
  processId: 'pazXumQI-HPH7iFGfTC-4_7biSnqz_U67oFAGry5zUY',
};

export const PIXL_TOKEN = {
  name: 'PIXL Token',
  ticker: 'PIXL',
  denomination: 6,
  logo: 'czR2tJmSr7upPpReXu6IuOc2H7RuHRRAhI7DXAUlszU',
  processId: 'DM3FoZUq_yebASPhgd8pEIRIzDW6muXEhxz5-JwbZwo',
};

export const WETH_TOKEN = {
  name: 'Ethereum-Wrapped ETH',
  ticker: 'wETH',
  denomination: 18,
  logo: 'RJ0om4TNkNM4nUsWC5KG3VkLf8HWfhsVm3tJBMke1Ws',
  processId: 'cBgS-V_yGhOe9P1wCIuNSgDA_JS8l4sE5iFcPTr0TD0',
};

export const WAR_TOKEN = {
  name: 'Wrapped AR',
  ticker: 'wAR',
  denomination: 12,
  logo: 'L99jaxRKQKJt9CqoJtPaieGPEhJD3wNhR4iGqc8amXs',
  processId: 'xU9zFkq3X2ZQ6olwNVvr1vUWIjc3kXTWr7xKQD6dh10',
};

export const USDT_TOKEN = {
  name: 'BSC-Wrapped USDT',
  ticker: 'wUSDT',
  denomination: 18,
  logo: '9vHNDNa6gHxhwndjL_SZcKvXzljarHRQxhHb3sgCeME',
  processId: '7j3jUyFpTuepg_uu_sJnwLE6KiTVuA9cLrkfOp2MFlo',
};

export const ARIO_TOKEN = {
  name: 'AR.IO Network',
  ticker: 'ARIO',
  denomination: 6,
  logo: 'GIayVyo49wof1hOtgLcJ_XAE6OuF5MeYiYsgu3z4gxk',
  processId: 'qNvAoz0TgcH7DMg8BCVn8jF32QH5L6T29VjHxhHqqGE',
};

export const GAME_TOKEN = {
  name: 'ArcAO',
  ticker: 'GAME',
  denomination: 18,
  logo: '-c4VdpgmfuS4YadtLuxVZzTd2DQ3ipodA6cz8pwjn20',
  processId: 's6jcB3ctSbiDNwR-paJgy5iOAhahXahLul8exSLHbGE',
};

export const PI_TOKEN = {
  name: 'Permaweb Index Token',
  ticker: 'PI',
  denomination: 12,
  logo: 'zmQwyD6QiZge10OG2HasBqu27Zg0znGkdFRufOq6rv0',
  processId: '4hXj_E-5fAKmo4E8KjgQvuDJKAFk9P2grhycVmISDLs',
};

export const DUMDUM_TOKEN = {
  name: 'Dumdum',
  ticker: 'DUMDUM',
  denomination: 18,
  logo: 'uKrcXiZgJg32Q1JmOyjSWlvfR0saFn-yUMHMw79lfsE',
  processId: 'jtGHIv6MRIwUSlxVUTDwX7X0gYEGKQynIqvkelIOdL4',
};

export const AVAILABLE_TOKENS = [
  WAR_TOKEN,
  AO_TOKEN,
  ARIO_TOKEN,
  USDT_TOKEN,
  USDC_TOKEN,
  TRUNK_TOKEN,
  LLAMA_TOKEN,
  PIXL_TOKEN,
  WETH_TOKEN,
  GAME_TOKEN,
  PI_TOKEN,
  DUMDUM_TOKEN,
];
